...
module.exports = async function (config = 'conventional', commits) {
  const file = builtInConfig.hasOwnProperty(config) ? builtInConfig[config] : config
  const cwd = builtInConfig.hasOwnProperty(config) ? __dirname : process.env.GITHUB_WORKSPACE
